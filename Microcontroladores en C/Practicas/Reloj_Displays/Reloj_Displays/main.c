...
void I2C_Parar(){//Genera la condición de parada del I2C.
	TWCR = (1<<TWINT) | (1<<TWEN) | (1<<TWSTO);
}

uint8_t I2C_Escribir_Byte(uint8_t Dispositivo,uint8_t Direccion, uint8_t Dato){//Escribe un byte en dispositivo que acepte dirección y dato.
...
uint8_t I2C_Escribir_Bytes(uint8_t Dispositivo, uint8_t Direccion, uint8_t* Dato, uint8_t longitud)//Escribe una cadena de bytes en dispositivo que acepte dirección y datos.
{
	if(Inicio_I2C_Write(Dispositivo)) return 1;
	I2C_Escribir(Direccion);
...
uint8_t Inicio_I2C_Read(uint8_t Direccion){//Lee el byte de un dispositivo,
	TWCR = 0;//Resetear registro de control.
	TWCR|=(1<<TWINT)|(1<<TWSTA)|(1<<TWEN);//Enviar condición de Inicio.
	while (!(TWCR & (1<<TWINT)));//Esperar a la bandera de inicio
	if ((TWSR & 0xF8) != START){
...
uint8_t I2C_LEER_ACK(void){//Lee un dato mandando reconocimiento ACK para seguir leyendo datos.
	Regreso_ACK:
	TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWEA);//Limpiar banderas para comenzar la transmición de la dirección.
	while (!(TWCR & (1<<TWINT)));//Esperar el ACK.
...
uint8_t I2C_LEER_NACK(void){//Lee un dato mandando reconocimiento NACK para dejar de leer datos.
	Regreso_NACK:
	TWCR = (1<<TWINT)|(1<<TWEN);//Limpiar banderas para comenzar la transmición de la dirección.
	while (!(TWCR & (1<<TWINT)));//Esperar el ACK.
...
uint8_t I2C_Escribir_Direccion(uint8_t Dispositivo,uint8_t Direccion){//Escribe en dato en un dispositvo I2C.
	if(Inicio_I2C_Write(Dispositivo)) return 1;
	I2C_Escribir(Direccion);
	I2C_Parar();
	return 0;
...
uint8_t I2C_LEER_Byte(uint8_t Dispositivo,uint8_t Direccion){//Lee un byte de un dispositivo.
...
uint8_t I2C_LEER_Bytes(uint8_t Dispositivo,uint8_t Direccion,uint8_t* Datos, uint8_t longitud){//Lee una cadena de bytes de un dispositivo.
	I2C_Escribir_Direccion(Dispositivo,Direccion);
	if(Inicio_I2C_Read(Dispositivo)) return 1;
...
uint8_t Dir_RTC=0x68;//Dirección del modulo reloj de tiempo real.
uint8_t Leer_Horas(){//Leer las horas del reloj de tiempo real.
	return  I2C_LEER_Byte(Dir_RTC,02);
}

uint8_t Leer_Minutos(){//Leer los minutos del reloj de tiempo real.
	return  I2C_LEER_Byte(Dir_RTC,01);
}

uint8_t Leer_Segundos(){//Leer los segundos del reloj de tiempo real.
	return I2C_LEER_Byte(Dir_RTC,00);
}

void Escribir_Horas(uint8_t Horas){//Escribir las horas del reloj de tiempo real.
	I2C_Escribir_Byte(Dir_RTC,02,Horas);
}

void Escribir_Minutos(uint8_t Minutos){//Escribir los minutos del reloj de tiempo real.
	I2C_Escribir_Byte(Dir_RTC,01,Minutos);
}

void Escribir_Segundos(uint8_t Segundos){//Escribir los segundos del reloj de tiempo real.
	I2C_Escribir_Byte(Dir_RTC,00,Segundos);
}

void Inicializar_SPI(){//Inicializa el modulo SPI en salidas y configuración de registros.
...
void Cargar_Dato(){//Función que manda un pulso por SS.
	PORT_SPI|=(1<<DD_CS);//Pone en alto SS.
	for (uint8_t i=0;i<10;i++){}//Espera 10 ciclos máquina
	PORT_SPI&=!(1<<DD_CS);//Pone en bajo SS.
}
void Escribir_Dato(uint8_t Dato){//Función que manda un dato por SPI sin actualizar el SS.
	SPDR=Dato;
	while(!(SPSR & (1<<SPIF)));//Espera a que sea mandado el dato.
}
void Escribir_SPI_Byte(uint8_t Dato){//Función que Escribe un byte en un dispositivo SPI.
	SPDR=Dato;
	while(!(SPSR & (1<<SPIF)));//Espera a que sea mandado el dato.
	Cargar_Dato();
}
void Escribir_SPI_Word(uint16_t Dato){//Función que Escribe un byte en un dispositivo SPI.
	SPDR=*(&Dato+1);//Manda el Byte Alto.
	while(!(SPSR & (1<<SPIF)));//Espera a que sea mandado el dato.
	SPDR=*(&Dato);//Manda el Byte Bajo.
	while(!(SPSR & (1<<SPIF)));//Espera a que sea mandado el dato.
	Cargar_Dato();//
}

void Escribir(uint8_t Display, uint8_t Numero){
	SPDR=Display;//Manda el display a modificar.
	while(!(SPSR & (1<<SPIF)));//Espera a que sea mandado el dato.
	SPDR=Numero;//Manda el numero a poner.
	while(!(SPSR & (1<<SPIF)));//Espera a que sea mandado el dato.
	Cargar_Dato();//Carga la instrucción
}
...
uint8_t Digito[2];
void Obtener_Digitos(uint8_t x){//Función para descomponer un número en BCD en 2 dígitos.
	Digito[0]=x&(0x0F);
	Digito[1]=(x&(0xF0))>>4;
}
